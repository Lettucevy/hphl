...
static const std::map<std::string, std::map<std::string, std::string>> kEmbeddedDefaults = {
    {"en", {
        ...
        {"cli_ir_o_ignored", "--backend ir emits LLVM IR only (written to '{1}'); ignoring -o '{0}' (no executable is linked; use --backend llvm to build an executable)"},
    ...
    {"ptbr", {
        ...
        {"cli_ir_o_ignored", "--backend ir emite apenas LLVM IR (gravado em '{1}'); ignorando -o '{0}' (nenhum executável é linkado; use --backend llvm para gerar um executável)"},
    ...
    {"es", {
        ...
        {"cli_ir_o_ignored", "--backend ir solo emite LLVM IR (escrito en '{1}'); ignorando -o '{0}' (no se enlaza ningún ejecutable; use --backend llvm para generar un ejecutable)"},

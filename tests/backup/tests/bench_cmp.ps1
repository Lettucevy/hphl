# bench_cmp.ps1 — comparativo HP-HL (--backend llvm) vs C++ (g++ -O2)
#
...
# (tests/benchmarks/cpp), mede a média de N rodadas e imprime a tabela com o
# speedup de C++ sobre cada nível de otimização HP-HL. As saídas de cada par
# são comparadas (mesmo resultado = benchmark válido).
#
# Requisitos: hphlc compilado (make em compiler/), gcc/g++ no PATH (MSYS2 ucrt64).
...
# 3) validação: saída de cada par deve ser idêntica
Write-Host ""
...
# 4) tabela — $t2 = média do ÚLTIMO opt da lista (o mais alto); iterar
#    $results.Keys em ordem arbitrária (hashtable) corrompia a coluna O2.
#    ATENÇÃO: não usar variável chamada $opts — colide (case-insensitive) com
#    o parâmetro [string]$Opts e toda atribuição vira string ("0 2").
$optList = $Opts -split "," | ForEach-Object { [int]$_ }
Write-Host ""
Write-Host ("comparativo (media de {0} rodadas, ms) - 'int' HP-HL = int64, 'double' = float64" -f $Runs) -ForegroundColor Cyan
...
foreach ($name in $cases) {
    ...
    $t0 = $results[0][$name]
    $lastOpt = $optList[-1]
    $t2 = if ($results.ContainsKey($lastOpt) -and $results[$lastOpt].ContainsKey($name)) { $results[$lastOpt][$name] } else { 0.0 }
    $tc = $cppTimes[$name]
    if ($tc -le 0) { continue }
    $r0 = $t0 / $tc
    $r2 = $t2 / $tc
